...
async function addProjeto(idObj, data) {
  return fetch(`http://localhost:8080/user/addProject/${idObj}`, {
    method: 'POST',
    headers: {
...
function Home() {
  ...
  const handleSubmit = async e => {
    ...
    await addProjeto(obj.id, newProject)
      ...
      const getProjects = async () => {
        try {
          const response = await fetch(
            `http://localhost:8080/user/projects/${obj.id}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
